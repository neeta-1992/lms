<x-app-layout :class="[]">
    <x-jet-form-section :title="$pageTitle" :activePageName="$activePage" :buttonGroup="['logs', 'other' => [['text' => __('labels.cancel'), 'url' => routeCheck($route . 'payment-method-permissions')]]]" class="validationForm editForm" novalidate action="{{ routeCheck($route . 'payment-method-permissions-settings',$id) }}"
        method="post">
        @slot('form')
