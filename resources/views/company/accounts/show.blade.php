        ...
        <div class="container tableButtonInlineShow">
            <div class="row">
                ...
                <div class="col-md-12 ">
                    ...
                    <div class="alert alert-info alertMessage lodeHtmlData" role="alert" x-show="!getCookie('{{ $value->id ?? '' }}')">
                        <button class="close" type="button" data-dismiss="alert" aria-label="Close" x-on:click="setCookie('{{ $value->id ?? '' }}',true,1)">
                            <span aria-hidden="true">×</span>
                        </button>
                        <strong>{!! $value->alert_subject ?? '' !!}</strong>
                        {!! $value->alert_text ?? '' !!}</div>
                    @endforeach
                    @endif
                </div>
                    ...
                    <div class="row ">
                        <div class="col-md-4">
                            ...
                            <div class="columns">
                                ...
                                <div class="ui selection dropdown table-head-dropdown maindropdown accountmaindropdown" style="">
                                    <input type="hidden" x-bind:value=open /><i class="dropdown icon"></i>
                                    <div class="text" x-text='title ?? "{{ __('labels.account_information') }}"'>@lang('labels.account_information')</div>
                                    <div class="menu" style="">
                                        <div class="item  pl-2 pr-2" x-on:click='open = "account_information";'>@lang('labels.account_information')</div>
                                        <div class="item pl-2 pr-2" x-on:click='open = "payment_schedule_history"' x-show='open != "payment_schedule_history"'>@lang('labels.payment_schedule_history')</div>
                                        <div class="item" x-on:click='open = "payments_transaction_history"' x-show='open != "payments_transaction_history"'> @lang('labels.payments_transaction_history')</div>
                                        <div class="item" x-on:click='open = "policies_endorsments"' x-show='open != "policies_endorsments"'>@lang('labels.policies_endorsments')</div>
                                        <div class="item" x-on:click='open = "enter_payment"' x-show='open != "enter_payment"'>@lang('labels.enter_payment')</div>
                                        <div class="item" x-on:click='open = "notice_history"' x-show='open != "notice_history"'>@lang('labels.notice_history')</div>
                                        <div class="item" x-on:click='open = "account_alerts"' x-show='open != "account_alerts"'>@lang('labels.account_alerts')</div>
                                        <div class="item" x-on:click='open = "manual_status_changes"' x-show='open != "manual_status_changes"'>@lang('labels.manual_status_changes')</div>
                                        <div class="item" x-on:click='open = "enter_return_premium_commission"' x-show='open != "enter_return_premium_commission"'> @lang('labels.enter_return_premium_commission')</div>
                                        <div class="item" x-on:click='open = "assess_manual_fee"' x-show='open != "assess_manual_fee"'>@lang('labels.assess_manual_fee')</div>
                                        @if(auth()->user()->can('isAdminCompany') || auth()->user()->can('company'))
                                            @if ($data->suspend_status == 0)
                                                 <div class="item" x-on:click='open = "suspend"' x-show='open != "suspend"'>@lang('labels.suspend')</div>
                                            @else
                                                <div class="item"  x-on:click='open = "unsuspend"'  data-remodal-target="unsuspendModel">@lang('labels.unsuspend')</div>
                                            @endif
                                             
                                        
                                        @endif
                                      
                                        <div class="item" x-on:click='open = "add_new_policy"' x-show='open != "add_new_policy"'>@lang('labels.add_new_policy')</div>
                                        <div class="item" x-on:click='open = "add_endorsements"' x-show='open != "add_endorsements"'>@lang('labels.add_endorsements')</div>
                                        <div class="item" x-on:click='open = "state_settings"' x-show='open != "state_settings"'>@lang('labels.state_settings')</div>
                                        <div class="item" x-on:click='open = "notes"' x-show='open != "notes"'>@lang('labels.notes')</div>
                                        <div class="item" x-on:click='open = "quotes"' x-show='open != "quotes"'>@lang('labels.quotes')</div>
                                        <div class="item" x-on:click='open = "attachments"' x-show='open != "quotes"'>@lang('labels.attachments')</div>
                                        <div class="item" x-on:click='open = "e_signature"' x-show='open != "e_signature"'>@lang('labels.e_signature_')</div>
                                        <div class="item" x-on:click='open = "logs"' x-show='open != "logs"'>@lang('labels.logs')</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-md-12">
                                    <div class="columns d-flex justify-content-end">
                                        <div class="lodeHtmlData" x-bind:class="(open == 'attachments') ? 'd-flex' : 'd-none'">
                                            ...
                                            <button class="btn btn-default borderless" type="button" x-on:click="uploadAttachment()">@lang('labels.upload_attachment')</button>
                ...
                {{-- Account Information  --}}
                <div class="col-md-12" x-show="open == 'account_information' || open == 'unsuspend'">
                ...
                {{-- manual_status_changes --}}
                <div class="col-md-12 enterReturnPremiumCommission" x-show="open == 'enter_return_premium_commission'"></div>
                ...
                {{-- suspend --}}
                <div class="col-md-12" x-show="open == 'suspend'">
                    @include($viwePath . 'pages.suspend')
                </div>
                    ...
                    <x-table id="{{ $activePage }}-logs" :noToggle="true">
                        <thead>
                            <tr>
                                ...
                                <th class="" data-sortable="true" data-field="username" data-width="200">@lang('labels.username')</th>
