<x-form action="{{ routeCheck($route.'note-edit',['accountId'=>$typeId,'id'=>$id]) }}" method="post" x-data="{istask:false}"  x-effect="async () => {
    if(istask){
  ...
  @method('put')
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-3">
            <x-select :options="['Private' => 'Private', 'Public' => 'Public']" name="note_type" required class="ui dropdown"
                placeholder="Select {{ __('labels.type') }}" selected="{{ $data->note_type ?? '' }}" />
        </div>
    </div>
    <div class="form-group row">
