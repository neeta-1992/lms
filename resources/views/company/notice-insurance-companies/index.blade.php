<x-app-layout>
    <x-jet-form-section :buttonGroup="['logs','other' => [['text' => 'Cancel', 'class' => 'backUrl']]]" class="validationForm" novalidate action="{{ routeCheck($route . 'store') }}" method="post">
        @slot('form')
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="vertical-middle">
                            ...
                            @endphp
                            <x-jet-checkbox for="notice_of_cancelled_account_frequency{{ $week }}" labelText='{{ $week }}' class="changesetup setup_fees" name="notice_of_cancelled[frequency][]" id="notice_of_cancelled_account_frequency{{ $week }}" value="{{ $week }}" :checked="$checked" />
                            @endforeach
                        </td>
                        <td class="vertical-middle">
                            <div class="d-flex align-items-center">
        ...
        <div class="mb-3">
            <p class="fw-500" style="font-size: 13px;">The notice defaults below are the initial settings for agencies, general agencies and
            ...
            <div class="col-lg-12">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            @php
                            $noticesDescriptionText = noticesDescriptionText(['onDB'=>true,'select'=>'name']);
                            @endphp
                            ...
                            @endphp
                            <tr>
                                <td><a href="javascript:void(0)" class="noticeTemplateShow" data-url="{{ routeCheck('ajax.notice-template-data',['type'=>'insurance-company','action'=>($key ??  "")]) }}" data-notice-type="{{ $sendType ?? '' }}">{{ $row ?? '' }}</a></td>
                                <td>
