    ...
    @foreach($data as $key => $value)
        @php
           ...
           $menuHtml .= ' <li class="nav-item">
                            <a class="nav-link '.$activeClass.'" href="#'.$Id.'" onClick="return false">'.$accountId.'</a>
                        </li>';

            $sectionHtml .= '<div id="'.$Id.'" class="scrollingsection">
                            <h4 class="my-4">Account Number : '.$accountId.'</h4>
                            <p>'.$value->template.'</p>
                        </div>';
        @endphp
    @endforeach
