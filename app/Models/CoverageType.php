...
class CoverageType extends Model
{
    ...
    protected $fillable = ['user_id','name','account_type','account_active','cancel_terms','parent_id'];
    protected $encryptable = ['name','account_type','account_active'];
    ...
    public static function insertOrUpdate(array $array){
        ...
        $account_type    = !empty($array['account_type']) ? $array['account_type'] : '' ;
        $account_active  = !empty($array['account_active']) ? $array['account_active'] : '' ;
        $cancel_terms    = !empty($array['cancel_terms']) ? $array['cancel_terms'] : 0 ;
        ...
        try {
           if(!empty($id) || !empty($parentId)){
            ...
            if($getdata->wasRecentlyCreated == true){
                $msg =  __('logs.coverage_type.add',['name'=>$name]);
            }else{
                if(!empty($logsmsg)){
                    $msg = __('logs.coverage_type.edit',['name'=>$name])." ".$logsmsg;
                }
            }
            /*
             * Logs Save In @Log Model
             */
            !empty($msg) && Logs::saveLogs(['type'=>'coverage-type','onDB'=>$onDB,'user_id'=>$logId,'type_id'=>$getdata->id,'message'=>$msg]);
