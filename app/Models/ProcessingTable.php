...
use App\Models\Logs;
use App\Models\ProcessingTableFee;
use App\Traits\ModelAttribute;
use DB,Error,Arr;
class ProcessingTable extends Model
{
    use HasFactory;
    use ModelAttribute;
    ...
    /**
     * Get all of the processingTableFee for the feeTable
     *
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function processingTableFee()
    {
        return $this->hasMany(ProcessingTableFee::class, 'processing_table_id', 'id');
    }
       ...
       // $feeTable['onDB'] = $onDB;
        ...
        try {
            ...
            if ($getdata->wasRecentlyCreated == true) {
                $msg = __('logs.processing_table.add', ['name' => $name]);
            } else {
                if (!empty($logsmsg)) {
                    $msg = __('logs.processing_table.edit', ['name' => $name]) . " " . $logsmsg;
                }
            }
    ...
    private static function feeInsertOrUpdate(array $feeTable = null, $feeId = 0, $userId = 0, $name = "",$arrayInput=null)
    {
        ...
        if (!empty($from)) {
            foreach ($from as $key => $value) {
                ...
                $getdataRateTableFee = ProcessingTableFee::insertOrUpdate($array);
                if ($getdataRateTableFee->wasRecentlyCreated == true) {
                    $msg .= " <li> ".__('logs.processing_table.fee_add', ['name' => $name, 'from' => '<b> $' . $fromVal . '</b>',
                            'to' => '<b> $' . $toVal . '</b>'])."</li>";
                } else {
        ...
        if(!empty($deleteRows)){
            ...
            foreach ($deleteRows as $key => $value) {
                ...
                $deleteRowData  = ProcessingTableFee::getData(['id'=>$id])->first();
