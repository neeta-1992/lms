...
class PolicyTermsOption extends Model
{
    use HasFactory;
    use ModelAttribute;
   // use EncryptedAttribute;

    protected $fillable = ['user_id','days','parent_id'];
    protected $encryptable = ['days'];
    ...
    public static function insertOrUpdate(array $array){
        ...
        $days            = !empty($array['days']) ? $array['days'] : 0;
        ...
        try {
           if(!empty($id) || !empty($parentId)){
            ...
            if($getdata->wasRecentlyCreated == true){
                $msg =  __('logs.policy_cancel_terms_options.add',['days'=>$days]);
            }else{
                if(!empty($logsmsg)){
                    $msg = __('logs.policy_cancel_terms_options.edit',['days'=>$days])." ".$logsmsg;
                }
            }
            /*
