...
class ProspectContact extends Model
{
    ...
    protected $fillable = [
        'prospect_id','first_name',
      'middle_name','last_name','title','month',
      'day','email','telephone','extension','fax','notes','password','office','role'
    ];
    ...
    public static function insertOrUpdate(array $array){
        ...
        $prospectId      = !empty($array['prospect_id']) ? $array['prospect_id'] : null ;
        ...
        $password           = !empty($array['password']) ? $array['password'] : null ;
        $role               = !empty($array['role']) ? $array['role'] : null ;
        $office = !empty($array['office']) ? $array['office'] : null ;
        $titleArr           = !empty( $array['titleArr']) ? json_decode ( $array['titleArr'],true) : null ;

        if(empty($password)){
            $password = randomPasswordUser(Entity::AGENT);
        }
        ...
        try {
           if(!empty($id) || !empty($parentId)){
            ...
            if(!empty($msg) && $logMag == false){
                Logs::saveLogs(['type'=>$activePage,'onDB'=>$onDB,'user_id'=>$logId,'type_id'=>$getdata->entity_id,'message'=>$msg]);
            }
     ...
     public static function getData(array $array = null)
    {
        ...
        if (GateAllow('isAdminCompany')) {
         ...
         if (isset($array['office'])) {
            $model = $model->where('office',$array['office']);
         }
        return $model;
    }
}
