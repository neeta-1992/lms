...
class QuoteStatusSetting extends Model
{
    use HasFactory;
    use ModelAttribute;
    ...
    public static function insertOrUpdate(array $array){
        ...
        $user_id         = !empty($array['user_id']) ? $array['user_id'] : (auth()->user()?->id ?? 1);
        ...
        try {
           if(!empty($id) || !empty($parentId)){
            ...
            if($getdata->wasRecentlyCreated == true){
                $msg = __('logs.quote_status_setting.add',['name'=>$name]);
            }else{
                if(!empty($logsmsg)){
                    $msg = __('logs.quote_status_setting.edit',['name'=>$name])." ".$logsmsg;
                }
            }
            /*
